...
#local assembly fasta(scaffolds.fasta)
seqFILE = sys.argv[4]
...
def filledByGap(gapInfo):
	...
	if gapInfo[3] != "{}":
		...
		#Fill from the lognest contig
		for contigName in contig:
			alnLeft = [0] * 7
			alnRight = [0] * 7
			inScaf = 0
			alignedScafPos1 = 0
			contigInfo = contigRangeInfo[contigName]
			...
			for x in alignment:
				...
				if gapStart >= aln_start and gapEnd <= aln_end: 
					inScaf = 1
					start = 0
					cnt = ''
									...
									if C[i] == 'D': 
										if scafCur > gapEnd: #all gaps in deletion
											if (gapEnd - gapStart + 1) == int(cnt):
												...
												alignedScafPos1 =  0
												return [gapStart, gapStart, gapEnd, '', "Delete", 0, contigName]
				...
				#Check if alignment is close enough to the gap
			...
			if alnLeft[1] != 0 and alnRight[1] != 0:
				...
				if not checkIdentity(alnLeft[3], 0.8) or not checkIdentity(alnRight[3], 0.8):
					status = 'LowIdentity'
					continue
		...
		if alignedScafPos1 and filled:
			...
			return [gapStart, alignedScafPos1, alignedScafPos2, contig_seq[alignReadPos1-1: alignReadPos2], "Complete", gapEnd-gapStart+1, contigName]
	...
	if not filled and gapInfo[5] != "{}":
		...
		for contiggg in contig:
			contigInfo = contigRangeInfo[contiggg]
			...
			#Alignment identity
			if L and not checkIdentity(rightAln[3], 0.9):
				L = 0
				status = "PartialFailed_I"
				print "PartialL covered gap:", gapStart, "Contig Idnetity < 0.9:", contiggg
	...
	if not filled and gapInfo[7] != "{}":
		...
		for contiggg in contig:
			...
			#Alignment identity
			if R and not checkIdentity(leftAln[3], 0.9):
				R = 0
				status = "PartialFailed_I"
				print "PartialR covered gap:", gapStart, "Contig Idnetity < 0.9:", contiggg
	...
	if L and R:
		...
		if alignedScafPos2_L >= alignedScafPos1_R:
			...
			return [gapStart, alignedScafPos1_L, alignedScafPos2_R, contig_seqL+"N"*30+contig_seqR, "L+30N+R", gapEnd-gapStart+1, len(contig_seqL), len(contig_seqR), alignedScafPos2_L, alignedScafPos1_R, contigNameL+"_"+contigNameR]
		else:
			...
			ns = alignedScafPos1_R - alignedScafPos2_L - 1
			return [gapStart, alignedScafPos1_L, alignedScafPos2_R, contig_seqL + "N"*ns + contig_seqR, "L+N+R", gapEnd-gapStart+1, contigNameL + "_" + contigNameR]
			#return [gapStart, alignedScafPos1_L, alignedScafPos2_L, contig_seqL, "L+N+R", gapEnd-gapStart+1, contigNameL], [gapStart, alignedScafPos1_R, alignedScafPos2_R, contig_seqR, "L+N+R", gapEnd-gapStart+1, contigNameR]
	...
	#Only left side is fillable
		...
		return [gapStart, alignedScafPos1_L, alignedScafPos2_L, contig_seqL, m, gapEnd-gapStart+1, contigNameL]
	...
	#Only right side is fillable
		...
		return [gapStart, alignedScafPos1_R, alignedScafPos2_R, contig_seqR, m, gapEnd-gapStart+1, contigNameR]

	elif not filled:
		#filling.append([gapStart, gapStart, gapEnd, status, "X", gapEnd-gapStart+1, "X"])
		return [gapStart, gapStart, gapEnd, status, "X", gapEnd-gapStart+1, "X"]
...
with open(contigFILE, "r") as file:
	for line in file:
		info = line.split()
		contigRangeInfo[info[0]] = info


with open(gapFILE, "r") as file:
    for line in file:
        gapInfo.append(line.split())

#multithreading for each gap to extractfillin
filling = sorted(Parallel(n_jobs=16)(delayed(filledByGap)(info) for info in gapInfo), key = lambda s: s[0], reverse=True)
...
for i, gap_info in enumerate(gap_record):
