...
while read -r scaf1 scaf2 BXCnt; do
	...
	if [ -f $wd/scaffolds.fasta ]; then
		...
		python /opt/LAB_Scaffolding/CrossContigInfo.py $wd/Scaf1_aln.sam $sp1 $wd/Scaf2_aln.sam $sp2 $MappedLen $Identity >> $ConnectInfoFile
...
python /opt/LAB_Scaffolding/Scaffolding.py $ConnectInfoFile $Fasta

printf '[%02d:%02d:%02d] %s\n' $(($SECONDS/3600)) $(($SECONDS%3600/60)) $(($SECONDS%60)) "LAB-Scaffolding is done. Collecting final fasta..." >> $Rec
FA=`echo $(basename "${Fasta}") | awk '{split($1,s,".fa"); print s[1]"_LABScaffold.fa"}'`
...
printf '[%02d:%02d:%02d] %s\n' $(($SECONDS/3600)) $(($SECONDS%3600/60)) $(($SECONDS%60)) "LAB-scaffolding is finished. Concatenated fasta is at $DIR/$FA. Log: $DIR/NewScaf.log." >> $Rec
...
printf '[%02d:%02d:%02d] %s\n' $(($SECONDS/3600)) $(($SECONDS%3600/60)) $(($SECONDS%60)) "Renamed LAB-Scaffolded fasta is at $DIR/${FArename}_rename.fa" >> $Rec
