...
MappedLen=500
MappedIdentity=80
while getopts o:s:a:t:M:I: option
do
        case "${option}"
        in
        a) Fasta=${OPTARG};;
...
#Split Supernova's scaffold to contigs
echo "[00:00:00]" Removing Ns in input g-scaffolds > $Rec
...
Filling () {
	...
	fa=$DIR/$ScafName.fa
	python /opt/LAB_GapFilling/locateGap.py $fa > $DIR/gap_pos_$ScafName.txt
	...
	python /opt/LAB_GapFilling/contigRange.py $DIR/${ScafName}_contig.txt > $DIR/${ScafName}_contigRange.txt
	...
	python /opt/LAB_GapFilling/filling.py $DIR/${ScafName}_coveredContig.txt $DIR/${ScafName}_contigRange.txt $fa $snova $MappedLen $MappedIdentity > $DIR/${ScafName}_filling.log 
        mv $DIR/${ScafName}_filled_fix.fa $DIR/${ScafName}_filled.fa
	echo $DIR/${ScafName}_filled.fa >> $DIR/getFA.txt

	python /opt/LAB_GapFilling/locateGap.py  $DIR/${ScafName}_filled.fa > $DIR/gap_pos_${ScafName}_filled.txt
