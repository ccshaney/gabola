...
parser.add_argument('-r2', '--trimr2', dest="trim_r2", 
                    type=lambda x: False if (str(x).lower() in ['false', '0']) else True, 
                    default=True,
                    help='''(Optional) R2 trimming. For better mapping quality, choose to trim barcode (16 mer) and adapter (6+1 mer), overall 23 mer, from R2 prefix. If choose not to trim R2, set this parameter to False or 0. [default:True]''')
...
if os.path.exists(input_fastqs_folder):
    if os.path.isdir(input_fastqs_folder):
        ...
        # R2:
        if trimR2:
            keepBX_R2_trim = input_fastqs_folder + project_id + '_barcoded_R2_val_trim23mer_reformat.fq'
            runKeepBXR2Trim = "sh {}keepFQwithBXandReformat.sh {} {}".format(prog_path, trimR2_out, keepBX_R2_trim)
