...
if [ -d $wd ]; then
	...
	python /opt/LCB_GapFilling/getContig_byGap.py $wd $gapCnt $ContigLen $ContigCov > $wd/scaffolds_all.fasta
	...
	python /opt/LCB_GapFilling/contigRange.py $wd/filled/scaf${s}_contig.txt > $wd/filled/scaf${s}_contigRange.txt
	...
	python /opt/LCB_GapFilling/filling.py $wd/filled/scaf${s}_coveredContig.txt $wd/filled/scaf${s}_contigRange_byGap.txt $wd/scaffold${s}.fa $wd/scaffolds_all.fasta $MappedLen $MappedIdentity > $wd/filled/filling.log 
	...
	python /opt/LCB_GapFilling/locateGap.py $wd/filled/Scaf${s}_filled.fa > $wd/filled/gap_pos_scaf${s}_filled.txt
...
cat $DIR/getFA.txt | xargs -n 1 cat > $DIR/${FArename}_LCBFilled.fa
#rm $DIR/getFA.txt
if [ ! -z $Num ] && [ -f $RestofFA ]; then
	cat $RestofFA >> $DIR/${FArename}_LCBFilled.fa
#	rm $RestofFA
fi
printf '[%02d:%02d:%02d] %s\n' $(($SECONDS/3600)) $(($SECONDS%3600/60)) $(($SECONDS%60)) "Finished. Gap-filled FASTA is @$DIR/${FArename}_LCBFilled.fa" >> $Rec
